...
const nodeBb: DeepNode = {
  id: 5,
  parentId: 3,
  entities: [entityBb1, entityBb2],
  children: [],
};
...
describe("PredictionsPage", () => {
  it("should render taxonomy and predictions", async () => {
    ...
    NodeService.getNodes = jest
      .fn()
      .mockImplementationOnce(() => Promise.resolve(getNodesResponse));

    PredictionService.getPredictions = jest
      .fn()
    ...
    expect(wrapper.findAllComponents(PredictionCard)).toHaveLength(
      getPredictionsResponse.totalPredictions
    );
  });
  ...
  it("should allow annotating synonym prediction", async () => {
    ...
    NodeService.getNodes = jest
      ...
      .mockImplementationOnce(() =>
        Promise.resolve(getNodesResponseWithEntityA2)
      );
    ...
    PredictionService.getPredictions = jest
      ...
      .mockImplementationOnce(() => Promise.resolve(getPredictionsResponse))
      .mockImplementationOnce(() =>
        Promise.resolve(getPredictionsResponseWithoutAnnotatedPrediction)
      );

    PredictionService.patchPrediction = jest
      .fn()
      .mockImplementationOnce(
        (candidate: string, predictionPatch: PredictionPatch) => {
    ...
    await wrapper
      .findAllComponents(PredictionCard)[0]
      .vm.$emit("createEntity", postEntity);
    ...
    expect(wrapper.findAllComponents(PredictionCard)).toHaveLength(
      getPredictionsResponseWithoutAnnotatedPrediction.totalPredictions
    );
  });
  ...
  it("should allow annotating child prediction", async () => {
    ...
    const entityAc1: Entity = {
      id: 12,
      nodeId: 7,
      name: "Ac-1",
      matchesCount: 0,
    };
    const nodeAc: DeepNode = {
      id: 7,
      parentId: 0,
      entities: [entityAc1],
      children: [],
    ...
    const getNodesResponseWithChildNodeAc = [
      nodeAWithChildNodeAc,
      nodeB,
      nodeC,
    ];
    ...
    NodeService.getNodes = jest
      ...
      .mockImplementationOnce(() => Promise.resolve(getNodesResponse))
      .mockImplementationOnce(() =>
        Promise.resolve(getNodesResponseWithChildNodeAc)
      );

    NodeService.postNode = jest
      .fn()
